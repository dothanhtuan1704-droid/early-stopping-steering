...
\textbf{1.~Within-Run Norm Offset:} Let $S_8^{(t)} = \frac{1}{N} \sum_{i=1}^N \|h_8^{(t)}(x_i)\|_2$ denote the single-step mean pre-hook hidden scalar norm across $N=50$ prompts at single-token decoding step $t \ge 1$ (post-prefill). We record the within-run norm difference before and after vector addition within the same forward pass:
\begin{equation}
