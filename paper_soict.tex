...
% --- LNCS Page Optimization & Float Alignment Controls ---
...
\renewcommand\subsection{\@startsection{subsection}{2}{\z@}%
                                     {-3.5pt \@plus -1pt \@minus -1pt}%
                                     {1.5pt \@plus 1pt \@minus 1pt}%
                                     {\normalfont\normalsize\bfseries}}
\makeatother
\setlength{\floatsep}{1.5pt plus 1pt minus 1pt}
\setlength{\textfloatsep}{1.5pt plus 1pt minus 1pt}
\setlength{\intextsep}{1.5pt plus 1pt minus 1pt}
...
\setlength{\abovedisplayshortskip}{0.5pt plus 1pt minus 1pt}
\setlength{\belowdisplayshortskip}{0.5pt plus 1pt minus 1pt}
...
Our main contributions are: (i)~\textbf{ViHaluEval-Medical Benchmark:} A benchmark of 14,700 expert-structured Vietnamese clinical Q\&A pairs (14,674 core pairs) derived from the official Vietnamese National Drug Formulary (2nd Edition, 2018) \cite{ref6}, featuring a primary-API partition disjoint test split ($N_{\text{test}}=500$). (ii)~\textbf{Dual-Horizon Evaluation Protocol:} Comprehensive evaluation under natural completion ($T=800$) and stress testing ($T=200$), supported by a 50-sample manual formulary audit manifest (\texttt{human\_evaluation\_completed\_50.csv}) and quality filtering logs (\texttt{removed\_26\_samples\_manifest.json}). (iii)~\textbf{Activation-Norm Dynamics:} Within-run and independent norm tracking at Layer~8, confirming immediate cessation of direct displacement post-cutoff ($\Delta_{\text{norm}}=0.00$) and no statistically significant residual offset ($p=0.1050$). (iv)~\textbf{Directional Specificity Controls:} Negative steering (62.80\%), label-permutation placebo (68.60\%), and $N=100$ isotropic random vectors ($55.82\% \pm 4.15\%$), establishing $z_{\text{sep}}=5.20$ standard deviations separation above random perturbation. (v)~\textbf{Statistical Validation \& Ablation:} Paired $2\times 2$ binomial McNemar testing ($p=0.0055$ at $T=200$; $p=0.00086$ at $T=800$) and strength ablation ($\alpha_0 \in \{15.0, 18.0, 20.0\}$).
...
\end{equation}
Continuous Steering ($K=\infty$) applies $\alpha(t) = \alpha_0$ for all $t \ge 1$. Cumulative executed dose depends on temporal window $K$, initial strength $\alpha_0$, and actual generated completion tokens post-prefill $L_i = N_{\text{new},i}$ (where $N_{\text{new},i}$ denotes single-token decoding steps executed post-prefill). Let $m = \min(K, \max(0, L_i))$. In the case of early end-of-sequence termination prior to step $K$, executed dose scales as $D_{\text{cutoff}} = m \cdot |\alpha_0|$, and $D_{\text{decay}} = |\alpha_0|\left[ m - \frac{m(m-1)}{2K} \right]$. Phase activation norms (e.g., baseline active phase mean $\bar{H}_{8,\text{active}}=52.88$, post-16 phase mean $\bar{H}_{8,\text{post16}}=52.20$) are evaluated across two measurement phases demarcated at step 16: active phase ($t \in [1, \min(L_i, 16)]$) and post-16 phase ($t \in [17, L_i]$). Aggregation follows a formal two-stage procedure: for each prompt $i$ with $L_i \ge 1$, we first compute the prompt-level mean norm across valid generated token steps $t \in \mathcal{T}_{i,\text{phase}}$, $\bar{h}_{8,i}^{\text{phase}} = \frac{1}{|\mathcal{T}_{i,\text{phase}}|} \sum_{t \in \mathcal{T}_{i,\text{phase}}} \|h_8^{(i)}(t)\|_2$, then compute the whole-phase sample average $\bar{H}_{8,\text{phase}} = \frac{1}{N_{\text{phase}}} \sum_{i=1}^{N_{\text{phase}}} \bar{h}_{8,i}^{\text{phase}}$. Sequences terminating early via EOS ($L_i \le 16$) contribute to the active phase over their generated tokens and are excluded from the post-16 denominator $N_{\text{post}}$ to avoid zero-padding distortion. Nominal Matched Cumulative Dose Control serves as a constant per-token dose rate control setting per-token coefficient $\alpha_{\text{match}} = 0.0425 \alpha_0$ ($0.7650$ for $\alpha_0=18.0$), scaling executed dose with generated completion length:
\begin{equation}
D_{\text{executed}} = L_i \cdot |\alpha_{\text{match}}|
\end{equation}
...
\begin{lstlisting}[language=Python,basicstyle=\ttfamily\scriptsize,aboveskip=2pt,belowskip=2pt,breaklines=true,columns=flexible]
class SteeringHookHandler:
    def __init__(self, v_steer, alpha_0=18.0, K=16, schedule="linear_decay"):
        if schedule not in ("linear_decay", "hard_cutoff", "continuous"):
    ...
    def hook_fn(self, module, input, output):
        ...
        self.t += 1
        if self.schedule == "linear_decay": a_t = self.alpha_0 * max(0.0, 1.0 - (self.t-1)/self.K)
        elif self.schedule == "hard_cutoff": a_t = self.alpha_0 if self.t <= self.K else 0.0
        elif self.schedule == "continuous": a_t = self.alpha_0
...
\textbf{1.~Within-Run Norm Offset:} Let $\mathcal{P}_t = \{i \mid L_i \ge t\}$ denote the set of active prompts executing single-token decoding step $t \ge 1$ (post-prefill), with $N_t = |\mathcal{P}_t|$ active sequences. The single-step mean pre-hook hidden scalar norm across active prompts at step $t$ is $S_8^{(t)} = \frac{1}{|\mathcal{P}_t|} \sum_{i \in \mathcal{P}_t} \|h_8^{(t)}(x_i)\|_2$. We record the within-run norm difference before and after vector addition within the same forward pass:
\begin{equation}
\Delta \text{norm}(t) = \frac{1}{|\mathcal{P}_t|} \sum_{i \in \mathcal{P}_t} \left( \|\hat{h}_8^{(t)}(x_i)\|_2 - \|h_8^{(t)}(x_i)\|_2 \right)
\end{equation}
where $h_8^{(t)}$ is the pre-hook Layer~8 hidden tensor and $\hat{h}_8^{(t)} = h_8^{(t)} + \alpha(t) \cdot v_{\text{steer}}$ is the post-hook tensor. Under unsteered baseline conditions ($\alpha(t)=0$ for all $t$), the unmodified whole-phase mean activation magnitude is $\bar{H}_{8,\text{active}} = 52.88$ during active decoding $t \in [1, 16]$ and $\bar{H}_{8,\text{post16}} = 52.20$ during post-16 decoding $t \in [17, L_i]$ across $N=50$ prompts. Continuous steering ($K=\infty$) induces persistent norm elevation ($\Delta_{\text{norm}} = +1.90$ active phase, $\Delta_{\text{norm}} = +2.51$ post-16 phase). Active injection at step $t=20$ under extended Linear Decay schedules ($K=24, 32$) yields corresponding offsets ($\Delta_{\text{norm}}^{(20)}=+1.12$ for $K=24, \alpha=3.75$; $\Delta_{\text{norm}}^{(20)}=+1.98$ for $K=32, \alpha=7.31$). Post-cutoff ($t > K$), $\alpha(t)=0$ yields $\Delta_{\text{norm}} = 0.00$ by construction, confirming immediate cessation of direct vector injection.

\textbf{2.~Independent Cross-Condition Post-Cutoff Norm Comparison:} To evaluate post-cutoff activation behavior across independent runs, we compare prompt-level mean pre-hook norms under Hard Cutoff against independent unsteered baseline completions across $N=50$ paired prompt completions (\texttt{independent\_activation\_trajectories.json}). For prompt $i \in [1, N]$, let $L_{i,\text{cutoff}}$ and $L_{i,\text{base}}$ denote the total single-token autoregressive decoding steps executed post-prefill ($L_i = T_{\text{gen},i} - 1$) until sequence termination (defined as emitting a ChatML stop token \texttt{<|im\_end|>} / \texttt{<|endoftext|>} at step $t = L_i$ or reaching \texttt{max\_new\_tokens}). For each sequence $i$, the post-cutoff measurement window evaluates decoding steps $t \in [17, L_i]$. The prompt-level post-cutoff mean pre-hook norm is $\bar{h}_{8,i}^{\text{cutoff}} = \frac{1}{|\mathcal{T}_{i,\text{post}}|} \sum_{t=17}^{L_{i,\text{cutoff}}} \|h_8^{(t)}(x_i)\|_2$ evaluated over $|\mathcal{T}_{i,\text{post}}| = \max(0, L_{i,\text{cutoff}} - 16)$ post-cutoff steps, and similarly $\bar{h}_{8,i}^{\text{base}} = \frac{1}{L_{i,\text{base}} - 16} \sum_{t=17}^{L_{i,\text{base}}} \|h_8^{(t)}(x_i)\|_2$. Sequences terminating at or before step 16 ($L_i \le 16$) have an empty post-16 step set ($\mathcal{T}_{i,\text{post}} = \emptyset$) and are excluded from post-cutoff analysis, yielding $N_{\text{post}}=50$ valid paired prompt completions. Prompt-level post-cutoff pre-hook norms under Hard Cutoff ($51.84$) exhibit no statistically significant difference relative to the independent baseline ($52.20$, mean prompt-level difference $-0.36$, sample SD $= 1.54$, paired $t$-test $p = 0.1050 > 0.05$, $95\%$ CI $[-0.80, +0.08]$). While no mean norm difference was detected by this test at Layer~8 post-cutoff, we explicitly note that $p > 0.05$ establishes non-significance rather than mathematical proof of strict equivalence.
...
\vspace{-2pt}
\begin{flushleft}
\scriptsize Note: Evaluated on \texttt{test\_ids.json} ($N_{\text{test}}=500$, \texttt{max\_new\_tokens=200}) via \texttt{analyze\_all\_new\_results.py}. Continuous ($K{=}\infty$) performance is reported at optimal scale $\alpha_0=18.0$. Nominal Matched Dose sets per-token coefficient $\alpha_{\text{match}} = 0.0425 \alpha_0$.
\end{flushleft}
\end{table}
...
\vspace{-2pt}
\begin{flushleft}
\scriptsize Note: Evaluated on \texttt{test\_ids.json} ($N=500$) via \texttt{eval\_prefix\_disentanglement\_fast.py} across sequence truncation boundaries $T \in \{50, 100, 200, 400, 800\}$.
\end{flushleft}
\end{table}

\textbf{Formulary Adjudication \& Metric Proxy Scope.} Benchmark reference answers ($y^+$) were constructed directly from official drug monographs \cite{ref6}. A stratified random audit sample of $N=50$ test questions (\texttt{human\_evaluation\_completed\_50.csv}) evaluated directly against official drug monographs confirmed 94.0\% (47/50) strict monograph alignment, documenting 3/50 non-alignments due to dosage snippet truncation or administration route ambiguity. Automated BERTScore RefPref evaluates model outputs relative to these formulary-grounded reference answers. We explicitly treat RefPref as an automated semantic preference proxy, acknowledging that direct clinician-adjudicated scoring of generated model completions across clinical evaluations remains essential future work.
...
\setlength{\itemsep}{0.2pt plus 0.3pt minus 0.1pt}
