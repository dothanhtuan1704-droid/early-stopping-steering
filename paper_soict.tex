...
\end{equation}
Layer-wise linear probing identified module block $j^*=8$ (the 9th residual block of Qwen2.5-7B, $d=3,584$) as exhibiting peak separation on the validation split ($N_{\text{val}}=2,223$), selected as the intervention layer.

\subsection{Temporally Bounded Intervention Schedules}
During autoregressive generation, let $t \in \{1, 2, \dots, T_{\text{gen}}\}$ denote the post-prefill decoding step index. At layer $j^*=8$, the modified hidden state $\hat{h}_{j^*}^{(t)}$ is computed as:
\begin{equation}
...
class EarlyStoppingSteeringHook:
    def __init__(self, v_steer: torch.Tensor, alpha_0: float = 18.0, K: int = 16, schedule: str = "linear_decay"):
        ...
        self.step = 0  # reset per completion; step 0 corresponds to prefill

    def reset(self):
        self.step = 0

    def __call__(self, module, inputs, output):
        if output[0].shape[1] > 1:  # prefill phase remains unsteered
            return output
        self.step += 1
        if self.step > self.K:
            return output
        if self.schedule == "hard_cutoff":
            coeff = self.alpha_0
        elif self.schedule == "linear_decay":
            coeff = self.alpha_0 * (1.0 - (self.step - 1) / self.K)
