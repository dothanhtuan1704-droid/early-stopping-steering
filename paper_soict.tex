...
\end{equation}
Continuous Steering ($K=\infty$) applies $\alpha(t) = \alpha_0$ for all $t \ge 1$. Cumulative executed dose depends on temporal window $K$, initial strength $\alpha_0$, and actual generated decoding steps post-prefill $T_{\text{gen}}-1$. Let $m = \min(K, \max(0, T_{\text{gen}} - 1))$. In the case of early end-of-sequence termination prior to step $K$, executed dose scales as $D_{\text{cutoff}} = m \cdot |\alpha_0|$, and $D_{\text{decay}} = |\alpha_0|\left[ m - \frac{m(m-1)}{2K} \right]$. Phase activation norms (e.g., baseline active norm 52.88, post-16 norm 52.20) are evaluated across two measurement phases demarcated at step $B=16$: active phase ($t \in [1, 16]$) and post-16 phase ($t > 16$). Aggregation follows a two-stage procedure: for each sequence $i$, we first compute the prompt-level mean norm across valid generated token steps $t \in T_{i,\text{phase}}$, $\bar{S}_{8,i}^{\text{phase}} = \frac{1}{|T_{i,\text{phase}}|} \sum_{t \in T_{i,\text{phase}}} \|h_8^{(i)}(t)\|_2$, then average across prompts $\text{Norm}_{\text{phase}} = \frac{1}{N_{\text{phase}}} \sum_{i=1}^{N_{\text{phase}}} \bar{S}_{8,i}^{\text{phase}}$. Sequences terminating early via EOS ($T_i \le B$) contribute to the active phase over their generated tokens and are excluded from the post-16 denominator $N_{\text{post}}$ to avoid zero-padding distortion. Nominal Matched Cumulative Dose Control serves as a constant per-token dose rate control setting per-token coefficient $\alpha_{\text{match}} = 0.0425 \alpha_0$ ($0.7650$ for $\alpha_0=18.0$), scaling executed dose with generated sequence length:
\begin{equation}
