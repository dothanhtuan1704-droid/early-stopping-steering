...
\textbf{1.~Within-Run Norm Offset:} Let $S_8^{(t)} = \frac{1}{N_t} \sum_{i=1}^{N_t} \|h_8^{(t)}(x_i)\|_2$ denote the single-step mean pre-hook hidden scalar norm across $N_t = |\{i \mid L_i \ge t\}|$ active prompts at single-token decoding step $t \ge 1$ (post-prefill), excluding early-terminated sequences ($L_i < t$). We record the within-run norm difference before and after vector addition within the same forward pass:
\begin{equation}
\Delta \text{norm}(t) = \frac{1}{N_t} \sum_{i=1}^{N_t} \left( \|\hat{h}_8^{(t)}(x_i)\|_2 - \|h_8^{(t)}(x_i)\|_2 \right)
\end{equation}
