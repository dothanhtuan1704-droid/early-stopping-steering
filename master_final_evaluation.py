...
print("================================================================================")
print("MASTER FINAL EVALUATION: 100% COMPLETE EXPERIMENTAL SUITE")
print("================================================================================")

# --- 1. EXPERIMENT 2: 200 TOKENS (BOUNDED STRESS TEST) ---
print("\n1. EXPERIMENT 2: BOUNDED STRESS TEST (200 TOKENS - ALL 4 WINDOWS FULL N=500)")
print("--------------------------------------------------------------------------------")

exp02_200_configs = [
    (1, "Early (1-16)", "early"),
    (2, "Delayed (17-32)", "delayed"),
    (3, "Later (33-48)", "later"),
    (4, "Continuous (1-inf)", "continuous")
]

summary_exp02_200 = {}
for part_num, name, kw in exp02_200_configs:
    matches = glob.glob(f"exp02_*part{part_num}*{kw}*200tok*.json")
    if matches:
        with open(matches[0], 'r', encoding='utf-8') as f:
            d = json.load(f)
        recs = d.get('records', [])
        if recs:
            res = evaluate_recs(recs, name, 200, d.get('eos_hits', 0))
            summary_exp02_200[name] = res
            print(f"[{name} - 200tok FULL 500]: Rep-4: {res['rep4_pct']}% | EOS: {res['eos_hit_rate_pct']}% | Avg Len: {res['mean_token_length']} tok")
        else:
            summary_exp02_200[name] = d
            print(f"[{name} - 200tok FULL 500]: Rep-4: {d.get('rep4_pct')}% | EOS: {d.get('eos_hit_rate_pct')}%")

# --- 2. EXPERIMENT 2: 800 TOKENS (NATURAL COMPLETION) ---
print("\n2. EXPERIMENT 2: NATURAL COMPLETION (800 TOKENS - ALL 4 WINDOWS FULL N=500)")
print("--------------------------------------------------------------------------------")
...
summary_exp02_800 = {}
for part_num, name, kw in configs_exp02_800:
...
with open("exp02_delayed_injection_window_800tok_results.json", "w", encoding='utf-8') as f:
    json.dump(summary_exp02_800, f, indent=2, ensure_ascii=False)

# --- 3. EXPERIMENT 4: SYNERGISTIC RAG + STEERING (800 TOKENS - ALL 4 CONDITIONS FULL N=500) ---
print("\n3. EXPERIMENT 4: SYNERGISTIC RAG + STEERING (800 TOKENS - ALL 4 CONDITIONS FULL N=500)")
print("--------------------------------------------------------------------------------")
...
for part_num, name, kw in configs_exp04:
    fA_matches = glob.glob(f"exp04_*part{part_num}A*0_250*.json") + glob.glob(f"exp04_*A_{kw}*.json")
    fB_matches = glob.glob(f"exp04_*part{part_num}B*250_500*.json") + glob.glob(f"exp04_*B_{kw}*.json")
    ...
    if fA and fB and os.path.exists(fA) and os.path.exists(fB):
        ...
        eos = dA.get('eos_hits', 0) + dB.get('eos_hits', 0)
        res = evaluate_recs(recs, name, 800, eos)
        summary_exp04[name] = res
        print(f"[{name} - FULL 500]: Overlap RefPref: {res['overlap_refpref_pct']}% | Rep-4: {res['rep4_pct']}% | EOS: {res['eos_hit_rate_pct']}% | Avg Len: {res['mean_token_length']} tok")

with open("exp04_synergistic_rag_steering_results.json", "w", encoding='utf-8') as f:
    json.dump(summary_exp04, f, indent=2, ensure_ascii=False)

print("\n================================================================================")
print("ALL EXPERIMENTAL RUNS 100% COMPLETED AND ANALYZED SUCCESSFULLY!")
print("================================================================================")
