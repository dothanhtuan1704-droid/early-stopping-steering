...
def process_merge_exp04(condition_name):
    partA_pattern = f"exp04_*A_{condition_name.lower()}*_800tok*.json"
    partB_pattern = f"exp04_*B_{condition_name.lower()}*_800tok*.json"
    ...
    print(f"\nMerging Exp04 {condition_name}: Loaded {len(records_A)} (Part A) + {len(records_B)} (Part B) = {len(all_500)} total samples.")
...
conditions = ["baseline", "steering_alone", "hybrid_rag_alone", "hybrid_rag_plus_steering"]
merged_exp04 = {}

for c in conditions:
    res = process_merge_exp04(c)
    if res:
        merged_exp04[c] = res

if merged_exp04:
    out_file = 'exp04_synergistic_rag_steering_results.json'
    with open(out_file, 'w', encoding='utf-8') as f:
        json.dump(merged_exp04, f, indent=2, ensure_ascii=False)
    print(f"\n✅ Merged Exp04 unified summary saved to '{out_file}'.")
else:
    print("\nNo Exp04 result files found yet on disk to merge.")
