...
| **`02_Table1_Bounded_Stress_Test_T200/`** | • `phase-6b-v2-bertscore-based-clinical-correctness.ipynb`<br>• `Kaggle_Phase6B_v2_BERTScore_Clinical.ipynb` | **Table 1 (Panel A)**<br>(Stress testing $T=200$) | Baseline 73.20% $\rightarrow$ Linear Decay **77.40%** (+4.20 pp, McNemar $p=0.0055$). |
| **`03_Table1_Natural_Completion_T800/`** | • `gpu-experiment-2a-primary-natural-completion-eval.ipynb`<br>• `Exp10_Baseline_PartA/B.ipynb`<br>• `Exp10_HardCutoff_NoPen_PartA/B.ipynb`<br>• `Exp10_LinearDecay_NoPen_PartA/B.ipynb` | **Table 1 (Panel B)**<br>(Natural completion $T=800$) | Baseline 65.40% $\rightarrow$ Hard Cutoff **70.60%** (+5.20 pp, McNemar $p=0.00086$, 100% EOS hit rate). |
