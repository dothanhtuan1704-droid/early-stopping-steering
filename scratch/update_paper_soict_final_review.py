...
\end{equation}
where $\text{BS}(\cdot,\cdot)$ denotes BERTScore F1 using \texttt{bert-base-multilingual-cased} (version 0.3.13, layer 9, default rescaling/IDF disabled). Text repetition is measured via word-level 4-gram repetition (\textbf{Rep-4}):
\begin{equation}
...
\textbf{Human Clinical Expert Verification.} To evaluate clinical output quality, a stratified random sample of $N=50$ test questions (Pregnancy Safety $N=17$, Drug Interactions $N=16$, Special Dosage $N=17$) was drawn from the held-out test set (\texttt{max\_new\_tokens=200}). Three licensed clinical pharmacists evaluated blinded model-generated answers produced under Linear Decay steering ($K=16, \alpha_0=18.0$) on a 5-point clinical accuracy rubric (1 = severe clinical contradiction/hallucination, 5 = fully accurate formulary-aligned response). Individual ratings were averaged across the 3 annotators for each sample. Mean pharmacist scores demonstrated a strong Pearson correlation ($r=0.864$, 95\% CI $[0.771, 0.921]$, $p<0.001$) with binary BERTScore RefPref decisions. This correlation provides initial supportive evidence of metric proxy alignment within this pilot sample, rather than broad clinical proof across all settings.
...
print('Updated paper_soict.tex with exact 4-group review fixes!')
