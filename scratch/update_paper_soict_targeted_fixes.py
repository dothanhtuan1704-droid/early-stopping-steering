...
\begin{abstract}
Small Language Models (SLMs) offer potential for privacy-preserving clinical decision support but frequently exhibit domain-specific hallucinations in resource-scarce languages like Vietnamese---such as miscalculating pediatric dosages or violating pregnancy contraindications. We propose Early-Stopping Activation Steering, a framework evaluating temporally bounded activation steering on Qwen2.5-7B-Instruct at Layer~8 with Hard Cutoff and Linear Decay schedules restricting vector injection to the first $K{=}16$ decoding steps ($t \in [1, 16]$, influencing output tokens $y_2 \ldots y_{17}$). Under high-cap natural completion (\texttt{max\_new\_tokens=800}), Hard Cutoff ($K{=}16$) achieves 70.60\% reference-preference accuracy (+5.20~pp over baseline, Holm-adjusted $p_{\text{adj}}=0.0026$) with 100\% EOS hit rate, accompanied by a slight decrease in BERTScore F1 (0.6695 vs 0.6779) and a minor increase in 4-gram repetition (3.74\% vs 3.68\%). Under bounded-generation stress testing (\texttt{max\_new\_tokens=200}), Linear Decay steering ($K{=}16$) increases accuracy from 73.00\% to 77.20\% (+4.20~pp, exact McNemar $p=0.0055$, 95\% Bootstrap CI $[+1.40, +7.00]$~pp), outperforming non-oracle BM25 RAG by +8.60~pp with zero context overhead. Evaluating combined Steering with Hybrid RAG yields 80.40\% RefPref accuracy (+1.00~pp additive gain over RAG alone, paired counts $a=392, b=5, c=10, d=93$, exact McNemar $p=0.3018$) while maintaining comparable low repetition (3.61\%). Descriptive placebo controls ($N{=}100$ random vectors, $55.82\%{\pm}4.15\%$) establish a $+5.15\sigma$ separation above isotropic random direction perturbation.
...
However, continuous activation steering ($K=\infty$) induces persistent hidden-state norm elevation over long output sequences, associated with cumulative activation displacement. In this paper, we present \textbf{Early-Stopping Activation Steering}, a framework evaluating temporally bounded intervention via Hard Cutoff and Linear Decay schedules restricting vector injection to the first $K=16$ decoding steps ($t \in [1, 16]$, influencing tokens $y_2 \ldots y_{17}$).

Our main contributions are: (i)~\textbf{ViHaluEval-Medical Benchmark:} A benchmark of 14,674 expert-structured Vietnamese clinical Q\&A pairs derived from the official Vietnamese National Drug Formulary (2nd Edition, 2018) \cite{ref6}, featuring a question-disjoint test split ($N_{\text{test}}=500$). (ii)~\textbf{Dual-Evaluation Paradigm \& Sample Human Validation:} Comprehensive evaluation covering high-cap natural completion (\texttt{max\_new\_tokens=800}, EOS Hit = 99.80\%--100.00\%) and bounded-generation stress testing (\texttt{max\_new\_tokens=200}), supported by sample pharmacist ratings ($N=50$) showing a strong correlation ($r=0.864, p<0.001$) with reference-preference decisions within this sample. (iii)~\textbf{Empirical Activation-Norm Dynamics:} Hidden-state activation tracking ($\|\hat{h}_8^{(t)}\|_2$) demonstrating persistent norm elevation under continuous steering ($56.34$ vs $53.39$ baseline at $t=50$, $\Delta = +2.95$) and local Layer~8 residual norm relaxation back to baseline equilibrium ($53.39 \pm 0.64$, $\Delta = 0.00$) upon hook deactivation. (iv)~\textbf{Directional Specificity \& Multi-Random Placebo Controls:} Negative steering ($-v_{\text{steer}}$, 62.80\%), label-permutation placebo steering ($v_{\text{shuf}}$, 68.60\%), and $N=100$ independent random vectors ($55.82\% \pm 4.15\%$ SD) confirming $+v_{\text{steer}}$ ($77.20\%$) achieves an empirical rank of $1/101$ ($p_{\text{emp}} = 0.0099$) at $+5.15\sigma$ above random perturbation. (v)~\textbf{Reproducible Statistical Validation:} Exact paired $2\times 2$ binomial McNemar testing ($p=0.0055$, 95\% Bootstrap CI $[+1.40\text{ pp}, +7.00\text{ pp}]$ for bounded stress testing; exact McNemar $p=0.00086$, 95\% Bootstrap CI $[+2.40\text{ pp}, +8.20\text{ pp}]$ for natural completion). (vi)~\textbf{Controlled Factorial Ablation:} Controlled $3\times 3$ equal-initial-$\alpha_0$ matrix alongside a nominal matched cumulative dose control ($\alpha_{\text{match}}$).
...
All candidate Q\&A pairs underwent manual adjudication by 3 licensed Vietnamese clinical pharmacists ($\ge 5$ years clinical experience), cross-referencing reference answers against source monographs to verify factual accuracy, achieving high inter-annotator agreement (mean pairwise Cohen's $\kappa = 0.91$; Fleiss' $\kappa = 0.89$). Benchmark data is partitioned at the active pharmaceutical ingredient (API) level into a development split ($N_{\text{dev}}=12,495$ pairs, 85.15\%: 70.0\% training $N_{\text{train}}=10,272$ for vector estimation; 15.15\% validation $N_{\text{val}}=2,223$ for hyperparameter tuning) and a held-out test partition ($N_{\text{test\_partition}}=2,179$ pairs, 14.85\%), from which $N_{\text{test}}=500$ evaluation questions were sampled using stratified random sampling with fixed seed 42: Pregnancy Safety ($N=165$), Drug Interactions ($N=160$), and Special Dosage ($N=175$).
...
\end{equation}
where $\text{BS}(\cdot,\cdot)$ denotes BERTScore F1 using \texttt{bert-base-multilingual-cased} (layer 9, default rescaling/IDF disabled). Text repetition is measured via word-level 4-gram repetition (\textbf{Rep-4}):
\begin{equation}
...
\end{equation}
where $G_4(\hat{y}_i)$ denotes consecutive word 4-grams extracted via whitespace tokenization (\texttt{text.split()}), aggregated as sample macro-average across $N=500$ sequences. Across all 4,000 generated test outputs (2,000 natural completion + 2,000 bounded stress test), zero outputs ($0.00\%$) exceeded the 512-subword evaluator window limit (0\% truncation rate) and zero score ties ($0.00\%$) occurred.

\subsection{Experimental Setup \& Implementation Protocol}\label{sec:setup}
All experiments evaluate Qwen2.5-7B-Instruct \cite{ref4} loaded in 4-bit NormalFloat4 (NF4) double-quantization via BitsAndBytes on NVIDIA Tesla T4 GPUs (CUDA 12.1, PyTorch 2.2.0, Transformers 4.38.2). All generation runs use deterministic greedy decoding (\texttt{temperature=0.0}, \texttt{do\_sample=False}, \texttt{top\_p=1.0}, seed 42) with key-value caching enabled (\texttt{use\_cache=True}). ChatML template system instruction: ``\textit{\vn{Bạn là một chuyên gia y dược lâm sàng Việt Nam. Hãy trả lời câu hỏi y khoa dưới đây một cách chính xác, khách quan dựa trên Dược thư Quốc gia Việt Nam (2nd Edition, 2018).}}''. PyTorch forward hook attached to decoder block \texttt{model.model.layers[8]} handles prefill ($t=0$, sequence length $>1$, resetting \texttt{self.t=0}) and single-token step decoding ($t \ge 1$, sequence length $=1$, incrementing \texttt{self.t+=1}):
...
\midrule
Unsteered Baseline & 65.40 & \textbf{0.6779} & \textbf{100.00} & \textbf{3.68} & -- & -- \\
Continuous ($K{=}\infty$) & 68.60 & 0.6705 & 99.80 & \textbf{3.50} & $0.0365$ & $[+0.40, +6.00]$ \\
Linear Decay ($K{=}16$) & 67.80 & 0.6732 & 99.80 & 3.74 & $0.1189$ & $[-0.20, +5.20]$ \\
\textbf{Hard Cutoff ($K{=}16$)} & \textbf{70.60} & 0.6695 & \textbf{100.00} & 3.74 & \textbf{0.00086} & $\mathbf{[+2.40, +8.20]}$ \\
\bottomrule
\end{tabular}
\begin{flushleft}
\footnotesize Note: Paired contingency counts ($a,b,c,d$): Hard Cutoff (311, 16, 42, 131), Continuous (309, 18, 34, 139), Linear Decay (308, 19, 31, 142), where $b$: baseline win, $c$: steered win. 95\% CIs via $B=10,000$ bootstrap. Rep-4 evaluated via word-level 4-gram whitespace macro-average.
\end{flushleft}
\end{table}

\subsection{Controlled Bounded-Generation Stress Test (\texttt{max\_new\_tokens=200})}\label{sec:bounded_results}
Under bounded stress testing (\texttt{max\_new\_tokens=200}), Linear Decay steering ($K{=}16$) achieves the highest accuracy among tested configurations at \textbf{77.20\%} (386/500, +4.20~pp over baseline 73.00\%, exact McNemar $p=0.0055$, Holm $p_{\text{adj}}=0.0165$, 95\% CI $[+1.40, +7.00]$~pp), representing a net difference of 1 question over Hard Cutoff (77.00\%, 385/500), while 4-gram repetition increases slightly to 3.74\% (vs 3.67\% baseline, Table~\ref{tab:clinical_safety}).
...
\midrule
Unsteered Baseline & 73.00 & 0.7133 & 23.12 & \textbf{3.67} & -- & -- \\
Continuous ($K{=}\infty$) & 75.60 & 0.7133 & \textbf{23.36} & 3.90 & $0.0854$ & $[-0.20, +5.40]$ \\
Hard Cutoff ($K{=}16$) & 77.00 & \textbf{0.7151} & 23.16 & 3.89 & $0.0078$ & $[+1.20, +6.80]$ \\
\textbf{Linear Decay ($K{=}16$)} & \textbf{77.20} & 0.7150 & 23.21 & 3.74 & \textbf{0.0055} & $\mathbf{[+1.40, +7.00]}$ \\
\bottomrule
\end{tabular}
\begin{flushleft}
\footnotesize Note: Paired contingency counts ($a,b,c,d$): Linear Decay (349, 16, 37, 98), Hard Cutoff (349, 16, 36, 99), Continuous (347, 18, 31, 104). 95\% CIs via $B=10,000$ bootstrap. Rep-4 evaluated via word-level 4-gram whitespace macro-average.
\end{flushleft}
\end{table}
...
\end{tabular}
\begin{flushleft}
\footnotesize Note: Paired McNemar counts vs Steering ($+v_{\text{steer}}$): vs BM25 RAG ($b=16$ BM25 win, $c=59$ Steering win, $p=6.11\times 10^{-7}$); vs Hybrid RAG ($b=22$ Hybrid win, $c=27$ Steering win, $p=0.5682$). $^\dagger$\,Profiled in separate Kaggle T4 sessions.
\end{flushleft}
\end{table}

\subsection{Controlled Factorial Ablation Matrix}
A $3\times 3$ factorial ablation (Table~\ref{tab:equal_alpha_ablation}) holding initial $\alpha_0$ fixed across schedules alongside Nominal Matched Dose ($\alpha_{\text{match}} = 0.0425 \alpha_0$) confirms Linear Decay ($K=16$) achieves highest accuracy at $\alpha_0 = 18.0$ (77.20\% RefPref, 3.74\% Rep-4).
...
\textbf{Temporal Window Injection Shift Control.} Intervening early ($t \in [1, 16]$) under Hard Cutoff step injection ($\alpha_0=18.0$) achieves 3.89\% Rep-4 at 200 tokens (3.74\% at 800 tokens), whereas applying the Linear Decay schedule ($K=16$) yields 3.74\% at 200 tokens (3.74\% at 800 tokens). Delayed step injection ($t \in [17, 32]$) yields 3.92\% Rep-4 at 800 tokens, and later injection ($t \in [33, 48]$) drops RefPref to 72.80\% (baseline 73.00\%), confirming that early intervention is critical to shape initial semantic trajectory commitment.
...
\textbf{Human Clinical Expert Verification.} To evaluate clinical output quality, a stratified random sample of $N=50$ test questions (Pregnancy Safety $N=17$, Drug Interactions $N=16$, Special Dosage $N=17$) was drawn from the held-out test set. Three licensed clinical pharmacists evaluated blinded model-generated answers produced under Linear Decay steering ($K=16, \alpha_0=18.0$) on a 5-point clinical accuracy rubric (1 = severe clinical contradiction/hallucination, 5 = fully accurate formulary-aligned response). Individual ratings were averaged across the 3 annotators for each sample. Mean pharmacist scores demonstrated a strong Pearson correlation ($r=0.864, p<0.001$) with binary BERTScore RefPref decisions. This correlation provides initial supportive evidence of metric alignment within this sample, rather than broad clinical proof across all settings.

\textbf{Combined Hybrid RAG + Steering Evaluation.} Under natural completion (\texttt{max\_new\_tokens=800}), evaluating combined Hybrid RAG + Steering across full $N_{\text{test}}=500$ samples achieves \textbf{80.40\% RefPref accuracy} (+1.00~pp additive gain over standalone Hybrid RAG 79.40\%, paired counts $a=392, b=5, c=10, d=93$, exact binomial McNemar $p=0.3018$). Combined RAG + Steering maintains low 4-gram repetition Rep-4 (\textbf{3.61\%} vs 3.78\% RAG alone, 3.68\% Baseline) while producing concise clinical completions ($167.86$ tokens vs $171.82$ tokens RAG alone).

\section{Discussion \& Limitations}\label{sec:discussion}
Reference-preference accuracy is a semantic proxy for factual alignment rather than a clinician-adjudicated correctness rate. Equal initial $\alpha_0$ produces unequal cumulative doses across schedules (Section~\ref{sec:taxonomy}). Validation tuning ($N_{\text{eval}}=500$) identified highest performance at Layer~8 ($77.40\%$), cutoff $K=16$ ($77.40\%$), and scale $\alpha_0=18.0$ ($77.40\%$). Deactivating vector injection past step $K$ directly restores Layer~8 outputs to unsteered baseline magnitude ($53.39 \pm 0.64$, $\Delta_{\text{norm}}^{(50)}=0.00$), preventing persistent norm elevation ($\Delta=+2.95$ under continuous steering) locally at Layer 8, while full network residual propagation remains a structural hypothesis. Percentile bootstrap CIs ($B=10,000$) use paired question-index resampling. Isotropic placebo controls ($N=100$, $55.82\% \pm 4.15\%$) confirm $+v_{\text{steer}}$ ($77.20\%$) achieves an empirical rank of $1/101$ ($p_{\text{emp}}=0.0099$) at $+5.15\sigma$ separation. Under high-cap natural completion (\texttt{max\_new\_tokens=800}), Hard Cutoff ($K=16$) maintains 100.00\% EOS hit rate while improving preference accuracy (+5.20~pp over baseline: 70.60\% vs 65.40\%, exact binomial McNemar $p=0.00086$, $p_{\text{adj}}=0.0026$, 95\% Bootstrap CI $[+2.40\text{ pp}, +8.20\text{ pp}]$), accompanied by a slight decrease in BERTScore F1 (0.6695 vs 0.6779) and a minor increase in 4-gram repetition (+0.06~pp, 3.74\% vs 3.68\%). Continuous steering at 800 tokens exhibits 3.50\% Rep-4, indicating early stopping does not reduce repetition compared to continuous steering at long output lengths, but rather avoids continuous norm elevation while achieving higher preference accuracy.
...
\bibitem{ref16} Lin, S. et al.: TruthfulQA: Measuring how models mimic how human falsehoods. In: Proc. ACL, pp. 3214--3252 (2022)
\end{thebibliography}
...
print('Updated paper_soict.tex with exact targeted fixes!')
