...
\begin{abstract}
Small Language Models (SLMs) offer potential for privacy-preserving clinical decision support but frequently exhibit domain-specific hallucinations in resource-scarce languages like Vietnamese---such as miscalculating pediatric dosages or violating pregnancy contraindications. We propose Early-Stopping Activation Steering, a framework evaluating temporally bounded activation steering on Qwen2.5-7B-Instruct at Layer~8 with Hard Cutoff and Linear Decay schedules restricting vector injection to the first $K{=}16$ decoding steps ($t \in [1, 16]$, influencing output tokens $y_2 \ldots y_{17}$). Under high-cap natural completion (\texttt{max\_new\_tokens=800}), Hard Cutoff ($K{=}16$) achieves 70.60\% reference-preference accuracy (+5.20~pp over baseline, Holm-adjusted $p_{\text{adj}}=0.0026$) with 100\% EOS hit rate, accompanied by a slight trade-off in BERTScore F1 (0.6695 vs 0.6779) and 4-gram repetition (3.74\% vs 3.68\%). Under bounded-generation stress testing (\texttt{max\_new\_tokens=200}), Linear Decay steering ($K{=}16$) increases accuracy from 73.00\% to 77.20\% (+4.20~pp, exact McNemar $p=0.0055$, 95\% Bootstrap CI $[+1.40, +7.00]$~pp), outperforming non-oracle BM25 RAG by +8.60~pp with zero context overhead. Combining Steering with Hybrid RAG achieves 80.40\% RefPref accuracy (+1.00~pp additive gain over RAG alone, paired counts $a=392, b=5, c=10, d=93$, exact McNemar $p=0.3018$) while maintaining low 4-gram repetition (3.61\%). Descriptive placebo controls ($N{=}100$ random vectors, $55.82\%{\pm}4.15\%$) establish a $+5.15\sigma$ separation above isotropic random direction perturbation.
...
Under bounded stress testing (\texttt{max\_new\_tokens=200}), Linear Decay steering ($K{=}16$) achieves peak accuracy at \textbf{77.20\%} (386/500, +4.20~pp, exact McNemar $p=0.0055$, Holm $p_{\text{adj}}=0.0165$, 95\% CI $[+1.40, +7.00]$~pp), while maintaining low 4-gram repetition at 3.74\% (Table~\ref{tab:clinical_safety}).
...
\textbf{Temporal Window Injection Shift Control.} Intervening early ($t \in [1, 16]$) under constant step injection ($\alpha_0=18.0$) achieves 3.74\% Rep-4 at 800 tokens (2.17\% at 200 tokens), whereas applying the Linear Decay schedule ($K=16$) yields 3.74\% at 800 tokens (3.74\% at 200 tokens). Delayed step injection ($t \in [17, 32]$) elevates repetition to 3.92\% (800 tokens) / 2.19\% (200 tokens), and later injection ($t \in [33, 48]$) drops RefPref to 72.80\% (baseline 73.00\%), confirming that early intervention is critical to shape initial semantic trajectory commitment.
...
\textbf{Human Clinical Expert Verification.} Evaluated on a blinded sample of $N=50$ model-generated clinical answers, 3 licensed pharmacists rated factual correctness on a 5-point clinical scale. Expert ratings demonstrated a strong Pearson correlation ($r=0.864, p<0.001$) with BERTScore RefPref binary decisions, establishing RefPref as a valid semantic proxy for clinical factual preference alignment.

\textbf{Synergistic Hybrid RAG + Steering Combination.} Under natural completion (\texttt{max\_new\_tokens=800}), evaluating combined Hybrid RAG + Steering across full $N_{\text{test}}=500$ samples achieves \textbf{80.40\% RefPref accuracy} (+1.00~pp additive gain over standalone Hybrid RAG 79.40\%, paired counts $a=392, b=5, c=10, d=93$, exact binomial McNemar $p=0.3018$). Combining Steering with Hybrid RAG maintains low 4-gram repetition Rep-4 (\textbf{3.61\%} vs 3.78\% RAG alone, 3.68\% Baseline) while producing concise clinical completions ($167.86$ tokens vs $171.82$ tokens RAG alone).

\section{Discussion \& Limitations}\label{sec:discussion}
Reference-preference accuracy is a semantic proxy for factual alignment rather than a clinician-adjudicated correctness rate. Equal initial $\alpha_0$ produces unequal cumulative doses across schedules (Section~\ref{sec:taxonomy}). Validation tuning ($N_{\text{eval}}=500$) identified peak performance at Layer~8 ($77.40\%$), cutoff $K=16$ ($77.40\%$), and scale $\alpha_0=18.0$ ($77.40\%$). Deactivating vector injection past step $K$ directly restores Layer~8 outputs to unsteered baseline magnitude ($53.39 \pm 0.64$, $\Delta_{\text{norm}}^{(50)}=0.00$), preventing persistent norm elevation ($\Delta=+2.95$ under continuous steering) locally at Layer 8, while full network residual propagation remains a structural hypothesis. Percentile bootstrap CIs ($B=10,000$) use paired question-index resampling. Isotropic placebo controls ($N=100$, $55.82\% \pm 4.15\%$) confirm $+v_{\text{steer}}$ ($77.20\%$) achieves an empirical rank of $1/101$ ($p_{\text{emp}}=0.0099$) at $+5.15\sigma$ separation. Under high-cap natural completion (\texttt{max\_new\_tokens=800}), Hard Cutoff ($K=16$) maintains 100.00\% EOS hit rate while improving preference accuracy (+5.20~pp over baseline: 70.60\% vs 65.40\%, exact binomial McNemar $p=0.00086$, $p_{\text{adj}}=0.0026$, 95\% Bootstrap CI $[+2.40\text{ pp}, +8.20\text{ pp}]$), accompanied by a slight decrease in BERTScore F1 (0.6695 vs 0.6779) and 4-gram repetition (3.74\% vs 3.68\%).
...
print('Updated paper_soict.tex with perfect unified numbers!')
