...
Let $\mathcal{D}_{\text{train}} = \{(x_i, y_i^+, y_i^-)\}_{i=1}^{N_{\text{train}}}$ denote contrastive training triplets ($N_{\text{train}}=10,272$), where $x_i$ is a clinical prompt, $y_i^+$ is a factual reference answer, and $y_i^-$ is a verified counterfactual hallucination. Given an autoregressive language model with $L_m$ transformer layers, let $h_l(x_i, y_i) = \operatorname{LayerOutput}_l(x_i \oplus y_i)[:, -1, :]$ denote the hidden state at layer $l \in \{1, \dots, L_m\}$ extracted at the sequence-terminal token position ($[:, -1, :]$, corresponding to the ChatML sequence terminator `<|im_end|>' under Qwen2.5). The contrastive difference vector at layer $l$ is:
\begin{equation}
\Delta h_l^{(i)} = h_l(x_i, y_i^+) - h_l(x_i, y_i^-) \in \mathbb{R}^d.
\end{equation}
The raw steering direction $\mu_l$ is the sample mean of contrastive differences:
\begin{equation}
\mu_l = \frac{1}{N_{\text{train}}} \sum_{i=1}^{N_{\text{train}}} \Delta h_l^{(i)}, \qquad v_{\text{steer}}^{(l)} = \frac{\mu_l}{\|\mu_l\|_2}.
\end{equation}
Layer-wise linear probing identified Layer~8 (the 9th residual block of Qwen2.5-7B, $d=3,584$) as exhibiting the highest separation, selected as the intervention layer.

\subsection{Temporally Bounded Intervention Schedules}
During autoregressive generation, let $t \in \{1, 2, \dots, T_{\text{gen}}\}$ denote the post-prefill decoding step index. At layer $l^*=8$, the modified hidden state $\hat{h}_{l^*}^{(t)}$ is computed as:
\begin{equation}
\hat{h}_{l^*}^{(t)} = h_{l^*}^{(t)} + \alpha(t) \cdot v_{\text{steer}}^{(l^*)},
\end{equation}
where $\alpha(t)$ is governed by one of three temporal intervention schedules parameterized by initial strength $\alpha_0 \in \mathbb{R}^+$ and cutoff horizon $K \in \mathbb{N}^+$:
\begin{align}
\textbf{Continuous Steering ($K{=}\infty$):} \quad &\alpha_{\text{cont}}(t) = \alpha_0, \quad \forall t \ge 1, \\
\textbf{Hard Cutoff Steering:} \quad &\alpha_{\text{hard}}(t) = \begin{cases} \alpha_0, & 1 \le t \le K, \\ 0, & t > K, \end{cases} \\
\textbf{Linear Decay Steering:} \quad &\alpha_{\text{decay}}(t) = \begin{cases} \alpha_0 \left(1 - \frac{t-1}{K}\right), & 1 \le t \le K, \\ 0, & t > K. \end{cases}
\end{align}
For $t > K$, $\alpha(t) = 0$ by definition, leaving subsequent forward passes unmodified while preserving conditioned KV-cache states.
...
class EarlyStoppingSteeringHook:
    def __init__(self, v_steer: torch.Tensor, alpha_0: float = 18.0, K: int = 16, schedule: str = "linear_decay"):
        ...
        self.alpha_0, self.K, self.schedule, self.step = alpha_0, K, schedule, 0

    def __call__(self, module, inputs, output):
        self.step += 1
        if self.step > self.K:
            return output
        coeff = self.alpha_0 if self.schedule == "hard_cutoff" else self.alpha_0 * (1.0 - (self.step - 1) / self.K)
        h = output[0] if isinstance(output, tuple) else output
        h[:, -1, :] = h[:, -1, :] + coeff * self.v_steer
        return (h,) + output[1:] if isinstance(output, tuple) else h
\end{lstlisting}
\caption{PyTorch implementation of the early-stopping activation steering forward hook.}
\label{lst:hook}
\end{figure}

\section{Experimental Setup}\label{sec:setup}
\subsection{ViHaluEval-Medical Benchmark Construction}
ViHaluEval-Medical was constructed from the Vietnamese National Drug Formulary (2nd Edition, 2018). A raw candidate pool of 14,700 QA pairs was extracted across 3 clinical categories: Special Dosage Instructions (5,000 pairs, 34.07\%), Pregnancy \& Lactation Safety (4,885 pairs, 33.29\%), and Clinically Significant Drug Interactions (4,789 pairs, 32.64\%). A rigorous filtering pipeline eliminated 26 malformed records (14 regex formatting anomalies, 8 near-duplicate pairs, 4 corrupted font encodings), yielding 14,674 validated records.

The dataset is partitioned into an 85.15\% development split (12,495 pairs; partitioned into $N_{\text{train}}=10,272$ and $N_{\text{val}}=2,223$) and a 14.85\% primary-API-disjoint held-out partition ($N_{\text{test\_partition}}=2,179$). From this partition, a balanced evaluation sample of $N_{\text{test}}=500$ pairs was drawn (175 Special Dosage, 165 Pregnancy Safety, 160 Drug Interactions). An audit of $N=50$ randomly sampled training pairs against monographs by independent clinical reviewers confirmed 47/50 correct alignments (94.0\% accuracy, 95\% Clopper-Pearson exact CI $[83.5\%, 98.7\%]$).
...
\toprule
\textbf{Intervention Schedule} & \textbf{RefPref (\%)} & \textbf{BERT F1} & \textbf{ROUGE-L (\%)} & \textbf{Rep-4 (\%)} & \textbf{McNemar $p$} & \textbf{95\% CI (pp)} \\
\midrule
\multicolumn{7}{c}{\textit{\textbf{Panel A: Bounded Horizon Stress Test ($T=200$)}}} \\
\midrule
Unsteered Baseline & 73.20 & 0.7134 & 23.12 & \textbf{3.67} & -- & -- \\
Continuous ($K{=}\infty$) & 75.60 & 0.7133 & \textbf{23.36} & 3.90 & 0.0854 & $[-0.20, +5.00]$ \\
Hard Cutoff ($K{=}16$) & 77.00 & 0.7151 & 23.16 & 3.89 & 0.0078 & $[+1.00, +6.60]$ \\
...
\bibitem{ref6} Dược thư Quốc gia Việt Nam: Ban Dược thư Quốc gia, Nhà xuất bản Y học, Hà Nội (2018)
\bibitem{ref7} Hu, E.J. et al.: LoRA: Low-rank adaptation of large language models. In: Proc. ICLR (2022)
\bibitem{ref8} Turner, A. et al.: Activation addition: Steering language models without optimization. arXiv preprint arXiv:2308.10248 (2023)
\bibitem{ref9} Rimsky, N. et al.: Steering llama 2 via contrastive activation addition. arXiv preprint arXiv:2312.06681 (2023)
