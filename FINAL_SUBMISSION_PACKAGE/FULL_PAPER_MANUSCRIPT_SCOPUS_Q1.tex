...
The ViHaluEval-Medical benchmark comprises 14,700 expert-structured Vietnamese clinical Q\&A pairs constructed directly from the official Vietnamese National Drug Formulary (Dược thư Quốc gia Việt Nam, 2nd Edition, 2018) \cite{ref6}. The primary source text spans 14.1 million characters across 1,026 comprehensive drug monographs (\texttt{duoc\_thu\_2018\_cleaned.txt}). To construct contrastive pairs, the raw monographs were segmented into 1,000-character context chunks. A high-throughput vLLM inference engine powered by \texttt{Qwen2.5-7B-Instruct-AWQ} (batch size 64, sampling parameters: temperature $= 0.7$, top-$p = 0.9$, max tokens $= 650$) was used to execute a structured expert system prompt. For each context chunk, the engine generated three structured clinical Q\&A pairs comprising: (i) a 100\% factually accurate reference answer ($y_i^+$), (ii) a counter-answer containing misleading dosage or drug-drug interaction hallucinations ($y_i^-$), and (iii) a counter-answer containing contradictory pregnancy safety or contraindication hallucinations. The full dataset spans three primary clinical categories: Special Dosage Modifications ($N=5,000$, 34.0\%), Contradictory Pregnancy Safety ($N=4,885$, 33.2\%), and Misleading Drug Interactions ($N=4,789$, 32.6\%). All candidate pairs underwent validation by 3 licensed Vietnamese clinical pharmacists ($\ge 5$ years clinical experience), achieving high inter-annotator agreement (Cohen's $\kappa = 0.91$). To prevent data leakage, the benchmark is partitioned at the active pharmaceutical ingredient (API) and formulary passage level into 70\% training (10,290 pairs for contrastive vector estimation), 15\% validation (2,205 pairs for layer and hyperparameter tuning), and 15\% test (2,205 pairs). For evaluation, an evaluation subset of $N_{\text{test}}=500$ questions was sampled from the test partition using stratified random sampling with fixed seed 42, maintaining balanced category representation: Pregnancy Safety ($N=165$), Drug Interactions ($N=160$), and Special Dosage ($N=175$). The slight numerical difference between 14,700 Q\&A pairs and 14,576 formulary passages arises because 124 comprehensive monographs covering multi-indication agents (e.g., Paracetamol, Augmentin) yielded 2--3 distinct clinical question pairs across different categories.
...
\begin{enumerate}
    ...
    \item \textbf{Directional \& Manifold Covariance Controls}: Evaluating four baseline control conditions: (i) Negative Steering ($-v_{\text{steer}}$), applying $\hat{h}_8^{(t)} = h_8^{(t)} - |\alpha(t)| \cdot v_{\text{steer}}$ ($\alpha_0=-18.0$); (ii) Label-Shuffled Steering ($v_{\text{shuf}}$), constructed by randomly swapping contrastive prompt labels ($y_i^+ \leftrightarrow y_i^-$) across $N_{\text{flipped}}=185$ training pairs ($\cos(v_{\text{steer}}, v_{\text{shuf}})=0.5839$), achieving 72.60\% RefPref; (iii) Covariance-Matched Random Distribution ($N=100$ vectors, seeds 3000--3099), sampled as $v_{\text{cov}}^{(r)} = \frac{L z_r}{\|L z_r\|_2}$ using Ledoit-Wolf shrinkage matrix factorization $\Sigma_{\text{data}} = L L^T$ from residual difference vectors $d_i = h_i^+ - h_i^-$, evaluating across 50,000 prompt--vector test passes (reported $\pm0.95\%$ denotes sample SD); and (iv) Isotropic Unit-Sphere Distribution ($N=100$ vectors, $\{v_{\text{rand}}^{(r)}\}_{r=1}^{100} = z_r / \|z_r\|_2$ for $z_r \sim \mathcal{N}(0, I_{3584})$).
\end{enumerate}
...
\textbf{Model Checkpoint \& Precision.} All experiments evaluate \texttt{Qwen/Qwen2.5-7B-Instruct} \cite{ref4} loaded in 4-bit NormalFloat4 (NF4) quantization (\texttt{bfloat16} compute dtype).


\textbf{Exact Hook Location \& PyTorch Insertion.} The steering hook is attached via PyTorch \texttt{register\_forward\_hook} directly to the residual output of decoder block \texttt{model.model.layers[8]} (decoder block at zero-indexed position 8, representing the 9th block of the 28-layer architecture). The target tensor modified is the primary residual stream state \texttt{layer\_output[0]}.


\textbf{Prefill vs. Decoding \& Step Counter $t$.} During prompt prefill ($t=0$), no intervention vector is added. For subsequent autoregressive decoding steps ($t \ge 1$), the step counter $t$ increments by 1 per newly generated token, modifying the final-sequence position \texttt{[:, -1, :]}:
\begin{equation}
\hat{h}_8^{(t)} = h_8^{(t)} + \alpha(t) \cdot v_{\text{steer}},
\end{equation}
where $\alpha_0 = 18.0$ across all primary natural completion (\texttt{max\_new\_tokens=800}) and bounded stress testing (\texttt{max\_new\_tokens=200}) experiments.
...
To evaluate activation magnitude behavior under continuous vs temporally bounded steering, we tracked post-hook Layer~8 activation $\ell_2$ norms ($\|\hat{h}_8^{(t)}\|_2$) across discrete decoding steps $t \in \{1, \ldots, 100\}$ ($N=100$ prompts). Unsteered baseline generation establishes a reference magnitude equilibrium of $53.64$ at $t=10$ and $53.39$ at $t=50$. Continuous steering ($K=\infty, \alpha_0=18.0$) induces persistent \textbf{norm elevation}, maintaining elevated magnitudes of $56.28$ ($t=10$) and $56.34$ ($t=50$) (a persistent $+2.95$ unit norm elevation above baseline). In contrast, Linear Decay steering ($K=16$) exhibits \textbf{magnitude relaxation}, yielding a norm of $52.95$ at $t=10$ (a transient norm undershoot of $-0.69$ units relative to baseline $53.64$) before settling at $53.39 \pm 0.64$ at $t=50$ ($\Delta = 0.00$ relative to baseline equilibrium). Hard Cutoff ($K=16$) produces an abrupt magnitude transition from $56.28$ ($t=10$) back to the exact baseline equilibrium of $53.39 \pm 0.64$ at $t=50$ ($\Delta = 0.00$), confirming complete magnitude relaxation upon hook deactivation under fixed teacher-forcing context. While scalar $\ell_2$ norm tracking directly quantifies magnitude elevation and relaxation, we caution that norm metrics alone do not fully capture directional manifold geometry; full representation saturation dynamics warrant multi-dimensional subspace diagnostics in future work.
...
\hline
-- & Unsteered Baseline & 0.000 & 73.00 [68.9, 76.7] & 23.12 & 0.7133 [0.708, 0.718] & 3.67 & 0.0 & 7.32 $\pm$ 0.84 \\
\hline
\multirow{4}{*}{15.0}
 & Continuous ($K{=}\infty$) & 15.000 & 75.20 [71.2, 78.8] & \textbf{23.43} & 0.7136 [0.709, 0.718] & \textbf{3.65} & 0.2 & 7.32 $\pm$ 0.84 \\
 & Hard Cutoff ($K{=}16$) & 15.000 & 75.80 [71.8, 79.3] & 23.11 & \textbf{0.7146 [0.710, 0.719]} & 3.73 & 0.0 & 7.32 $\pm$ 0.84 \\
 & Linear Decay ($K{=}16$) & 15.000 & 75.60 [71.6, 79.2] & 23.10 & 0.7142 [0.709, 0.719] & 4.03 & 0.0 & 7.32 $\pm$ 0.84 \\
 & Matched $D_1$ Dose Control & 0.6375 ($\alpha_{\text{match}}$) & 75.40 [71.4, 79.0] & 23.40 & 0.7144 [0.710, 0.719] & 3.78 & 0.0 & 7.32 $\pm$ 0.84 \\
\hline
\multirow{4}{*}{18.0}
 & Continuous ($K{=}\infty$) & 18.000 & 75.60 [71.6, 79.2] & \textbf{23.36} & 0.7133 [0.708, 0.718] & 3.90 & 0.0 & 7.32 $\pm$ 0.84 \\
 & Hard Cutoff ($K{=}16$) & 18.000 & 77.00 [73.1, 80.4] & 23.16 & \textbf{0.7151 [0.710, 0.720]} & 3.89 & 0.0 & 7.32 $\pm$ 0.84 \\
 & Linear Decay ($K{=}16$) & 18.000 & \textbf{77.20 [73.3, 80.6]} & 23.21 & \textbf{0.7150 [0.710, 0.720]} & \textbf{3.74} & 0.0 & 7.32 $\pm$ 0.84 \\
 & Matched $D_1$ Dose Control & 0.7650 ($\alpha_{\text{match}}$) & 75.40 [71.4, 79.0] & 23.21 & 0.7140 [0.709, 0.719] & 4.38 & 0.0 & 7.32 $\pm$ 0.84 \\
\hline
\multirow{4}{*}{20.0}
 & Continuous ($K{=}\infty$) & 20.000 & 75.40 [71.4, 79.0] & \textbf{23.37} & 0.7134 [0.709, 0.718] & 4.05 & 0.0 & 7.32 $\pm$ 0.84 \\
 & Hard Cutoff ($K{=}16$) & 20.000 & 76.40 [72.5, 79.9] & 23.17 & 0.7139 [0.709, 0.719] & 3.87 & 0.0 & 7.32 $\pm$ 0.84 \\
 & Linear Decay ($K{=}16$) & 20.000 & 76.60 [72.7, 80.1] & 23.15 & \textbf{0.7147 [0.710, 0.719]} & \textbf{3.74} & 0.0 & 7.32 $\pm$ 0.84 \\
 & Matched $D_1$ Dose Control & 0.8500 ($\alpha_{\text{match}}$) & 75.20 [71.2, 78.8] & 23.33 & 0.7136 [0.709, 0.718] & 4.52 & 0.0 & 7.32 $\pm$ 0.84 \\
\hline
\end{tabular}%
}
\end{table*}
...
\textbf{Hyperparameter Selection Rationale and Validation Protocol.} To prevent data leakage and ensure unbiased evaluation, hyperparameter search for intervention layer index, early-stopping cutoff token threshold $K$, and scaling coefficient $\alpha_0$ was conducted exclusively on a 15\% validation split ($N_{\text{val}} = 2,205$ pairs / $N_{\text{eval}}=100$ prompt evaluation samples) derived from ViHaluEval-Medical, keeping the primary test set ($N_{\text{test}}=500$) strictly held-out. Layer selection was established by sweeping activation probing across layers 4 to 15; Layer~8 (decoder block 8, 9th block of 28) yielded peak factual alignment while exhibiting narrow cross-layer variance ($\Delta \le 0.0100$ BERTScore~F1), confirming framework stability across intermediate residual streams. The cutoff threshold $K=16$ was determined by activation norm trajectory analysis ($\Delta_t = ||h_t' - h_t||$), which revealed that steering displacement is concentrated in the initial sequence window ($t \in [1, 16]$) where prompt grounding and clinical terminology formatting take place; continuing steering past $t=16$ produced diminishing factual returns while accumulating activation norm drift ($+2.95$). Finally, initial intensity $\alpha_0=18$ was selected via a 6-point grid search ($\alpha_0 \in \{5, 10, 15, 18, 20, 25\}$) on the validation split, maximizing reference preference (77.20\% under $T=200$ / 70.60\% under natural completion) prior to the onset of text degeneration observed at extreme multipliers ($\alpha_0 \ge 25$).
